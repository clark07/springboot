...
public class IDCardUtil {
	...
	public boolean verifyBirthdayCode(String code) {
		...
		try
		{
		 birthday=DateFormatter.parser(yearCode+month+dayCode, "yyyyMMdd");
		}
		catch(Exception ex)
		{
			return false;
		...
		if(birthday==null)
			return false;
		Date curDate=DateFormatter.parseryyyyMMdd(new Date());
		// 验证未满18岁
		int years=curDate.getYear()-birthday.getYear();
	...
	{
		Date birthday=DateFormatter.parser("10000101", "yyyyMMdd");
		IDCardUtil util=new IDCardUtil();
		System.out.print(util.verify("441624200005141955"));
	}
